...
#ifndef _parse_audio_loc_buf_H_
#define _parse_audio_loc_buf_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "parse_audio_loc_buf_memcore.h"

SC_MODULE(parse_audio_loc_buf) {
...
protected:
    ...
    // buffer instances
    parse_audio_loc_buf_memcore* buffer[BufferCount];
...
public:
    ~parse_audio_loc_buf();
    SC_CTOR(parse_audio_loc_buf) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "parse_audio_loc_buf_memcore_U_";
            name += szTmp;

            buffer[i] = new parse_audio_loc_buf_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "parse_audio_loc_buf.trace");
            // ports
