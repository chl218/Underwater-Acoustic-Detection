...
#ifndef _parse_audio_sig_buf_H_
#define _parse_audio_sig_buf_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "parse_audio_sig_buf_memcore.h"

SC_MODULE(parse_audio_sig_buf) {
...
protected:
    ...
    parse_audio_sig_buf_memcore* parse_audio_sig_buf_memcore_U;
...
public:
    ~parse_audio_sig_buf();
    SC_CTOR(parse_audio_sig_buf) : m_trace_file(0) {
        SC_METHOD(proc_memcore_addr);
        sensitive << i_address0 << t_address0 << iptr << tptr;

        parse_audio_sig_buf_memcore_U = new parse_audio_sig_buf_memcore("parse_audio_sig_buf_memcore_U");
        parse_audio_sig_buf_memcore_U->clk      ( clk );
        parse_audio_sig_buf_memcore_U->reset    ( reset );
        parse_audio_sig_buf_memcore_U->ce0      ( i_ce0 );
        parse_audio_sig_buf_memcore_U->we0      ( i_we0 );
        parse_audio_sig_buf_memcore_U->address0 ( memcore_iaddr );
        parse_audio_sig_buf_memcore_U->d0       ( i_d0 );
        parse_audio_sig_buf_memcore_U->q0       ( i_q0 );

        parse_audio_sig_buf_memcore_U->ce1      ( t_ce0 );
        parse_audio_sig_buf_memcore_U->we1      ( t_we0 );
        parse_audio_sig_buf_memcore_U->address1 ( memcore_taddr );
        parse_audio_sig_buf_memcore_U->d1       ( t_d0 );
        parse_audio_sig_buf_memcore_U->q1       ( t_q0 );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "parse_audio_sig_buf.trace");
            // ports
